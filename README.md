...
name (`gemini-3.1-flash-image` / `gemini-3-pro-image`). The tool contract
tells the model to only pass it when the human **explicitly asked** for a
specific model; otherwise the **per-user default** chosen in the `/ui`
dashboard applies, falling back to `flash`. `image_size` is `1K` (default), `2K`, or `4K`. Ids and
